...
- **Issue tracker** — where work is tracked, so `triage`/`to-spec`/`to-tickets` know whether to call `gh`, `glab`, write markdown under `docs/issues/`, or follow a workflow you describe. GitHub, GitLab, local markdown, or other.
